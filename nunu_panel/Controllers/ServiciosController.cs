        ...
        public List<ServicioProveedorModel>? GetServicios()
        {
            try
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    List<ServicioProveedorModel>? serv = JsonConvert.DeserializeObject<
        ...
        public IActionResult EliminarServicio(int idServicio)
        {
            ...
            if (response.StatusCode == HttpStatusCode.NoContent)
            {
                ...
                TempData["SuccessMensajeUsuario"] =
                    content != null ? $"Servicio {idServicio} eliminado con éxito!" : null;
                return Json(new { success = true, message = TempData["SuccessMensajeUsuario"] });
            }
            else
            {
                TempData["ErrorMensajeUsuario"] =
                    $"Error al eliminar el servicio desde la API. {response.StatusCode}";
